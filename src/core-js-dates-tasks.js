...
function getNextFriday(date) {
  let dayToFriday = 5 - date.getDay();
  if (dayToFriday <= 0) dayToFriday += 7;
  date.setDate(dayToFriday + date.getDate());
  return date;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (last - first) / 1000 / 60 / 60 / 24 + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 0; i < new Date(year, month, 0).getDate(); i += 1) {
    if (
      ...
      res.push(new Date(year, month - 1, i + 1).getDay());
    }
  }
  return res.length;
}
